...
describe("LandingPageForm Initial State Tests", () => {
  beforeEach(() => {
    cy.visit("http://65.0.29.107:8082/");
  });
...
describe("ExpenseForm", () => {
  beforeEach(() => {
    cy.visit("http://65.0.29.107:8082/"); // Adjust the path to where your component is rendered
  });
  ...
  it("should handle form submission and update state", () => {
    ...
    cy.window()
      ...
      .then((state) => {
        ...
        expect(state.transaction.transactionList.length).to.equal(1);
        expect(state.transaction.transactionList[0]).to.deep.include({
          name: newExpenseName,
